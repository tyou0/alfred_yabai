...
| `./release.sh` | Deploy + create GitHub release with auto-generated changelog (uses `gh` or `gmt`) |
...
| `release.sh` | GitHub release automation — uses `gh` when available, or `gmt` as fallback |
